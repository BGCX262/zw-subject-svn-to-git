...
var swfObj = {
    ...
    file_types: "*.avi;*.rmvb;*.mp4;*.wmv;*.swf;",
    file_types_description: "*.avi;*.rmvb;*.mp4;*.wmv;*.swf;",
...
function btdelrecources_click(courseid, name) {
    ...
    zwobj.data = { courseid: courseid, name: name, type: "视频列表" };
    ajaxData();
}

function v_click(parameters) {
    location.href = "/views/OnlineLearning/OLVideo.aspx?filepath=" + parameters;
}
...
function Ajax_GetCourseAndResource(data) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < len2; j++) {
            if ((j % 3) == 0) {
                html += '<tr>' +
                    '<td><span><img src="../../images/play_alt.png" width="17px" height="17px" /><a title="' + data.Data[i].Resourceses[j].Name + '" onclick="v_click(\'' +
                ...
                html += '<td><span><img src="../../images/play_alt.png" width="17px" height="17px"  /><a title="' + data.Data[i].Resourceses[j].Name + '" onclick="v_click(\'' +
                    encodeURI(data.Data[i].Resourceses[j].FilePath) + '\')">' + data.Data[i].Resourceses[j].Name.substr(0, 18) +
                    '...</a><button name="libtn" class="btn btn-primary btn-xs" data-toggle="modal" onclick="btdelrecources_click(\'' +
                    data.Data[i].Resourceses[j].CourseId + '\',\'' + data.Data[i].Resourceses[j].Name + '\')">删除</button></span></td>';
            }
